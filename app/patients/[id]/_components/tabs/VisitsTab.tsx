...
const VisitCard = ({
  ...
  return (
    <Card
      dir={dir}
      className="mb-4 border-l-4 shadow-md border-l-indigo-400 dark:border-l-indigo-600 border-indigo-100 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md transition-all duration-200"
    >
      <CardHeader
        className="py-3 bg-indigo-50/50 dark:bg-slate-800/50 rounded-t-lg cursor-pointer"
        onClick={() => toggleExpand(visit._id)}
      >
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            <div className="p-2 rounded-full bg-indigo-100 dark:bg-slate-700">
              {visitType.toLowerCase().includes("emergency") ? (
                <AlertCircle className="h-5 w-5 text-indigo-600 dark:text-slate-400" />
              ) : visitType.toLowerCase().includes("treatment") ? (
                <FileText className="h-5 w-5 text-indigo-600 dark:text-slate-400" />
              ) : (
            ...
            <div>
              <CardTitle className="text-base font-semibold text-indigo-800 dark:text-slate-300 flex items-center gap-2">
                {visit.title ||
                ...
                {visitType && (
                  <Badge
                    variant="outline"
                    className="mx-2 bg-indigo-50 dark:bg-slate-800 text-indigo-700 dark:text-slate-300 border-indigo-200 dark:border-slate-700"
                  >
                    {visitType}
                  </Badge>
                )}
              ...
              <div className="flex items-center mt-1 text-xs text-indigo-600 dark:text-slate-400">
          ...
          <div className="flex items-center gap-2">
            {visit.sectionData?.followUpDate && (
              <Badge
                variant="outline"
                className="bg-indigo-50 dark:bg-slate-800 text-indigo-700 dark:text-slate-300 border-indigo-200 dark:border-slate-700"
              >
            ...
            {visit.isDeleted ? (
              <Button
                ...
                size="sm"
                className="h-8 border-indigo-200 dark:border-slate-700 hover:bg-indigo-50 dark:hover:bg-slate-800 text-indigo-700 dark:text-slate-300"
              >
              ...
              <Button
                ...
                className="h-8 text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 border-red-200 dark:border-red-800/50 hover:bg-red-50 dark:hover:bg-red-900/30"
              >
            ...
            <ChevronDown
              className={`h-5 w-5 text-indigo-600 dark:text-slate-400 transition-transform ${
      ...
      <Card className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-indigo-100 dark:border-slate-800 shadow-xl transition-all duration-200">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <CardTitle className="text-xl text-indigo-800 dark:text-slate-300 flex items-center gap-2">
                <Calendar className="h-5 w-5 text-indigo-600 dark:text-slate-400" />
                {t("visitHistory")}
              </CardTitle>
              <CardDescription className="text-indigo-600 dark:text-slate-400 mt-1">
                {t("patientVisitsAndAppointments")}
              </CardDescription>
            </div>
            <Button
              className="bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-600/90 dark:hover:bg-indigo-700/90 text-white"
              onClick={() => setIsVisitDialogOpen(true)}
            >
              <PlusCircle className="h-4 w-4 mx-2" /> {t("addVisit")}
            </Button>
          </div>
        </CardHeader>
        ...
        <CardContent className="p-4">
          <Tabs defaultValue="active" className="w-full">
            <TabsList className="mb-4 bg-indigo-50 dark:bg-slate-800">
              <TabsTrigger
                value="active"
                className="flex items-center gap-1 data-[state=active]:bg-indigo-100 dark:data-[state=active]:bg-slate-700 data-[state=active]:text-indigo-800 dark:data-[state=active]:text-slate-200"
              >
                <CheckCircle2 className="h-4 w-4" />
                {t("active")}
                <Badge className="mx-1 bg-indigo-200 text-indigo-800 dark:bg-indigo-900/80 dark:text-slate-300">
                  {activeVisits.length}
                </Badge>
              </TabsTrigger>
              <TabsTrigger
              ...
              >
                ...
                {t("deleted")}
                <Badge className="mx-1 bg-red-100 text-red-800 dark:bg-red-900/80 dark:text-red-200">
