  ...
  return (
    ...
    >
      <div className="flex items-center mb-1 sm:mb-2">
        <Icon className="h-4 w-4 sm:h-5 sm:w-5 text-indigo-600 dark:text-slate-400 mx-1.5 sm:mx-2" />
        <span className="font-semibold text-indigo-800 dark:text-slate-200 text-sm sm:text-base">
          {label}
        </span>
      </div>
      <p className="text-gray-800 dark:text-slate-300 text-sm sm:text-base">
        {value || t("notProvided")}
      </p>
    </div>
  );
    ...
    <Card className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-indigo-100 dark:border-slate-800 shadow-xl transition-all duration-200">
      <CardHeader className="pb-3">
        <CardTitle className="text-xl text-indigo-800 dark:text-slate-300 flex items-center gap-2">
          <ClipboardList className="h-5 w-5 text-indigo-600 dark:text-slate-400" />
      ...
      <CardContent>
        {Object.entries(patient?.sectionData || {}).map(
          ([sectionKey, sectionValue]) => (
            <div key={sectionKey} className="mb-8">
              <div className="bg-indigo-50/50 dark:bg-slate-800/50 p-3 rounded-lg mb-4">
                <h3 className="text-lg font-semibold capitalize flex items-center gap-2 text-indigo-700 dark:text-slate-300">
              ...
              <Separator className="my-6 bg-indigo-100 dark:bg-slate-700" />
            </div>
          )
        )}
        ...
        {(!patient?.sectionData ||
          ...
          <div className="text-center p-8 bg-indigo-50/50 dark:bg-slate-800/50 rounded-lg">
            <FileText className="h-12 w-12 mx-auto text-indigo-300 dark:text-slate-600 mb-3" />
